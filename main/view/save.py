...
def loaddata_kdin(fac_number, fflow, hhead, dfkdin):
    ...
    for eff_size in eff_size_lst:
        ...
        eff_flow = (
            dfkdin.query(
                f"data_type =='EFF' and eff_rl == '{eff_size}' and fac_number == '{fac_number}'"
            )
    ...
    (
        ...
        eff_x_14,
        eff_x_15,
    ) = eff_x_list + [[0]] * (15 - len(eff_size_lst))
    ...
    (
        ...
        eff_y_14,
        eff_y_15,
    ) = eff_y_list + [[0]] * (15 - len(eff_size_lst))
    ...
    try:
        ...
        if x > x_max or x < x_min or y > y_max or y < y_min:
            ...
            imp_in_near_x_y1 = [i for i, value in enumerate(imp_list_fir) if value[0] == imp_near_x_y1[0] and value[1] == imp_near_x_y1[1]]
            imp_in_near_x_y1 = imp_in_near_x_y1[0]

            imp_list_fir[imp_in_near_x_y1] = 0, 0
            # print(im_size_lst[imp_in_near_x_y1])
            ...
            combiend_eff10 = list(zip(eff_data_x[9], eff_data_y[9]))
            combiend_eff11 = list(zip(eff_data_x[10], eff_data_y[10]))
            combiend_eff12 = list(zip(eff_data_x[11], eff_data_y[11]))
            combiend_eff13 = list(zip(eff_data_x[12], eff_data_y[12]))
            combiend_eff14 = list(zip(eff_data_x[13], eff_data_y[13]))
            combiend_eff15 = list(zip(eff_data_x[14], eff_data_y[14]))
            ...
            try:
                ...
                eff_list_fir.append(closest_point(combiend_eff10, target))
                eff_list_fir.append(closest_point(combiend_eff11, target))
                eff_list_fir.append(closest_point(combiend_eff12, target))
                eff_list_fir.append(closest_point(combiend_eff13, target))
                eff_list_fir.append(closest_point(combiend_eff14, target))
                eff_list_fir.append(closest_point(combiend_eff15, target))
            except:
                pass
            ...
            chart = get_plot(fac_number,name,im_size_lst,imp_data_x,imp_data_y,eff_size_lst,eff_x_list,eff_y_list,flow_input,Head_input,imp_output, eff_output, power, npshr_near_y)
            # print("Impeller dimension is %f mm" % (imp_output))
            # print('Power requirment = %f kW' %(power))
            # print("Efficiency is = %f percent" % (eff_output))
            # print('NPSHr is %f m'%npshr_near_y)
...
def get_plot(fac_number,name,im_size_lst,imp_data_x,imp_data_y,eff_size_lst,eff_x_list,eff_y_list,flow_input,Head_input,imp_output, eff_output, power, npshr_output):
...
dfkdin = pd.read_sql(
                    f'SELECT fac_number,flow,head,imp_dia,kw,npshr,data_type,model,eff,se_quence,eff_rl FROM factory_table where model_short = "KDIN" ORDER BY `factory_table`.`se_quence` ASC' , con=mydb)

loaddata_kdin('FAC-0001', 45, 45,dfkdin)